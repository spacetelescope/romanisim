...
from romanisim import log, util

from .parameters import V3IdlYAngle, n_pix, pixel_scale
from .wcs_utils import getWCS
...
def get_wcs(image, usecrds=True, distortion=None):
    ...
    if distortion is not None:
        ...
        wcs_dict = getWCS(
            world_pos=util.celestialcoord(world_pos),
            SCAs=sca,
            date=date.datetime,
...
def get_mosaic_wcs(mosaic, shape=None, xpos=None, ypos=None, coord=None):
    ...
    if type(mosaic) is not roman_datamodels.datamodels.MosaicModel:
        mosaic_node = roman_datamodels.datamodels.MosaicModel.create_fake_data(
            {"meta": mosaic}, shape=shape
        )
    else:
        mosaic_node = mosaic
    ...
    if shape is None:
        shape = (mosaic_node.data.shape[0], mosaic_node.data.shape[1])
        shape = mosaic_node.data.shape

    xcen = math.ceil(shape[1] / 2.0)
    ycen = math.ceil(shape[0] / 2.0)
    wcs = create_tangent_plane_gwcs((xcen, ycen), pixel_scale, world_pos)
    return wcs
...
def create_s_region(wcs, shape=None):
    ...
    xcorn, ycorn = (
        [bbox[0][0], bbox[0][1], bbox[0][1], bbox[0][0]],
