...
def getSkyLevel(bandpass, world_pos=None, exptime=None, epoch=2025, date=None):
    ...
    """
    if exptime is None:
        from .parameters import exptime
    ...
    sky_level[sky_level < 0] = 1.0e6

    # Interpolate in 2d on the table.
    s = sky_level.reshape(46, 42).transpose()
    xlat = sin_ecliptic_lat * 41
    xlon = abs(ecliptic_lon.wrap() / degrees) / 4.0
    ilat = int(xlat)
    ilon = int(xlon)
    xlat -= ilat
    xlon -= ilon
    sky_val = (
        s[ilat, ilon] * (1.0 - xlat) * (1.0 - xlon)
        + s[ilat, ilon + 1] * (1.0 - xlat) * xlon
        + s[ilat + 1, ilon] * xlat * (1.0 - xlon)
    ...
    if sky_val > max_sky:
        raise GalSimValueError(
            "world_pos is too close to sun. Would not observe here.", world_pos
        )
