...
infile = os.path.join(data_root, "sca_positions_20210204.txt")
...
sip_filename = os.path.join(data_root, "sip_20210204.txt")
...
def getWCS(world_pos, PA=None, date=None, SCAs=None, PA_is_FPA=False):
    ...
    date, SCAs, pa_fpa, pa_obsy = _parse_WCS_inputs(
        world_pos, PA, date, PA_is_FPA, SCAs
    )
    ...
    for i_sca in SCAs:
        ...
        header.extend(
            [
                ...
                (
                    "PA_OBSY",
                    pa_obsy / coord.degrees,
                    "position angle of observatory Y axis (deg)",
                ),
                (
                    "PA_FPA",
                    pa_fpa / coord.degrees,
...
def _parse_WCS_inputs(world_pos, PA, date, PA_is_FPA, SCAs):
    ...
    if not allowedPos(world_pos, date):
        raise GalSimError(
            "Error, Roman cannot look at this position on this date!"
        )
...
def _parse_sip_file(file):
    ...
    if not os.path.exists(file):
        raise OSError(
            "Cannot find file that should have Roman SIP coefficients: %s"
            % file
        )
...
def allowedPos(world_pos, date):
    ...
    min_ang = 90.0 - max_sun_angle
    max_ang = 90.0 + max_sun_angle
...
def bestPA(world_pos, date):
    ...
    north = coord.CelestialCoord(y_obs.ra, 90.0 * coord.degrees)
...
def convertCenter(
    world_pos, SCA, PA=None, date=None, PA_is_FPA=False, tol=0.5 * coord.arcsec
):
    """
    ...
    while shift_val > tol / coord.arcsec:
        ...
        fpa_cent = coord.CelestialCoord(
            fpa_cent.ra + delta_ra, fpa_cent.dec + delta_dec
        )
...
def findSCA(wcs_dict, world_pos, include_border=False):
    ...
    if not isinstance(wcs_dict, dict):
        raise TypeError(
            "wcs_dict should be a dict containing WCS output by galsim.roman.getWCS."
        )
