...
# Turn off FITS memmap for all regtests (affects FITSDiff)
# conf.use_memmap = False


@pytest.fixture(scope="session")
def artifactory_repos(pytestconfig) -> Tuple[str, str]:
...
def generate_artifactory_json(request, artifactory_repos):
    ...
    def artifactory_result_path() -> str:
    ...
    rtdata: RegtestData = postmortem(request, 'rtdata') or postmortem(request, 'rtdata_module')
    if rtdata:
        try:
            # The _jail fixture from ci_watson sets tmp_path
            cwd = Path(request.node.funcargs['tmp_path'])
        except KeyError:
            # The jail fixture (module-scoped) returns the path
            cwd = Path(request.node.funcargs['jail'])
        ...
        if rtdata.output and rtdata.output.exists():
            # Write the rtdata class out as an ASDF file
            path_asdf = cwd / f"{request.node.name}_rtdata.asdf"
            rtdata.to_asdf(path_asdf)

            # Generate an OKify JSON file
            pattern = rtdata.remote_results_path / rtdata.output.name
            okify_schema_pattern.append(pattern)
            okify_schema = generate_upload_schema(
                okify_schema_pattern,
                f"{rtdata.truth_remote.parent}/"
            )

            jsonfile = cwd / f"{request.node.name}_okify.json"
            ...
            upload_schema_pattern.append(jsonfile.absolute())
            ...
            jsonfile = cwd / f"{request.node.name}_results.json"
            with open(jsonfile, 'w') as fd:
                json.dump(upload_schema, fd, indent=2)


def generate_upload_schema(pattern: Union[str, List[str]], target: str, recursive: bool = False) -> Dict[
    str, List[str]]:
    """
    ...
    :param pattern: Specifies the local file system path to test results which should be uploaded to Artifactory.
    You can specify multiple artifacts by using wildcards or a regular expression as designated by the regexp property.
    :param target: Specifies the target path in Artifactory in the following format::

            [repository_name]/[repository_path]

    :param recursive: Specify whether to identify files listed in sub-directories for uploading. Default: `False`
    :returns: Dictionary specifying the upload schema
    """
...
def _rtdata_fixture_implementation(artifactory_repos: Tuple[str, str], envopt: str):
...
@pytest.fixture
def diff_astropy_tables() -> bool:
    """Compare astropy tables with tolerances for float columns."""

    def _diff_astropy_tables(result_path: str, truth_path: str, rtol: float = 1e-5, atol: float = 1e-7) -> bool:
