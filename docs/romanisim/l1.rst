...
L1 files may include a reference read, which for Roman is the reset read immediately
preceding the science resultants.  On board, this read is subtracted from each
of the science resultants and a constant ``data_encoding_offset`` is added, so that
the read noise does not clip against zero DN in the unsigned data transferred from
the spacecraft.  The reference read itself is downlinked
unmodified in the ``reference_read`` array, so the transformation is
invertible.  The primary purpose of this transformation is to improve data compression from
Roman; the reference read compresses poorly.
...
The reference read is treated separately from the science resultants: it does not appear in
``exposure.read_pattern``, which describes only the science resultants.  It is nevertheless
transmitted to the ground, so it *is* counted in ``exposure.nresultants``; with a reference read
``nresultants`` is therefore ``len(read_pattern) + 1`` rather than ``len(read_pattern)``.

We treat the reference read as being a zero-exposure time read.  The reference read
then accumulates no counts, cosmic rays, or persistence.  The reference read is otherwise
treated identically to other reads (e.g., it goes through the same inverse-linearity application).
