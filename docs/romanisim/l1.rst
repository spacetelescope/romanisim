...
Flight L1 files include a *reference read*: a single read of the detector taken
at the reset level, before any charge has accumulated.  On board, this read is
subtracted from each of the science resultants and a constant
``data_encoding_offset`` is added, so that the read noise does not clip against
zero DN in the unsigned data.  The reference read itself is downlinked
unmodified in the ``reference_read`` array, so the transformation is
invertible; ``romancal``'s ``dq_init`` step inverts it.  The transformation and
its inverse are available on their own as :func:`romanisim.l1.encode_reference_read`
and :func:`romanisim.l1.decode_reference_read`, for code that needs to apply or
undo the encoding without writing a file.

romanisim simulates this when ``reference_read`` is set (``--reference-read``
on the command line).  The reference read is carried through non-linearity,
IPC, gain, dark decay, read noise, and quantization alongside the resultants,
so it picks up its own realization of the read noise.  Because it is a single
read rather than an average of :math:`N` reads, it carries the full read noise;
and because it is subtracted from every resultant, that one realization is
imprinted coherently on the whole ramp.  Reproducing this correlated pedestal
term is the main reason to simulate a reference read rather than simply writing
the arrays.

The reference read is *not* a resultant: it does not appear in
``exposure.read_pattern``, is not counted in ``exposure.nresultants``, and has
no plane in the data quality array.

Each pixel is read essentially immediately after it is reset, so the reference
read is given no exposure time: it accumulates no counts, no cosmic rays, and
no persistence.  It nevertheless goes through the non-linearity curve like any
other read, since the charge in the pixel at the reset level is real.  Charge
deposited before the reference read would in any case be subtracted back out of
every resultant on board, so the only thing this approximation costs is a
second-order shift along the linearity curve for the small fraction of pixels a
cosmic ray would have struck in that interval.
