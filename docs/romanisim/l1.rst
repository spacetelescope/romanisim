...
.. math:: P(t) = A \\frac{1}{1+\exp(\\frac{-(x-x_0)}{\\delta x})} (\\frac{x}{x_0})^\\alpha (\\frac{t}{1000 \\mathrm{s}})^\\gamma \\, .

Here :math:`P(x, t)` is the rate in electrons per second that the pixel records :math:`t` seconds following receiving a total number of electrons :math:`x`.  The parameters :math:`A, x_0, \\delta x, \\alpha, \\gamma` may vary from pixel to pixel, though are presently fixed to global constants.
